...
output:
  ...
  pdf_document:
    toc: no
vignette: >
  %\VignetteIndexEntry{masscleaner_install}
...
`tinyTools` has been renamed as `tinytools`. So please uninstall it before you install `masscleaner`. **And don't use it any more.**
...
You can just use the `devtools` or `remotes` to install `masscleaner`. 
...
```
Error: Failed to install 'masscleaner' from GitHub: HTTP error 403. API rate limit exceeded for 171.66.10.237. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.)
```
...
`raster` is a package in [CRAN](https://cran.r-project.org/web/packages/raster/index.html), try to install it first and then install `masscleaner`.
...
`Cario` is a package in [CRAN](https://cran.r-project.org/web/packages/Cairo/index.html), try to install it first and then install `masscleaner`.
