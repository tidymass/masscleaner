...
#'
#' This function lists all the conflicts between packages in the masscleaner
...
#' existing code.
#' @return masscleaner_conflicts
#' @export
#' @examples
#' masscleaner_conflicts()
masscleaner_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", masscleaner_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "masscleaner_conflicts")
}

masscleaner_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "masscleaner_conflicts()"
  )
...
#' @export
print.masscleaner_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(masscleaner_conflict_message(x))
}
